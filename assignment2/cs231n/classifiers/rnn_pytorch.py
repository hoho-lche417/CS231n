...
class CaptioningRNN:
    ...
    def loss(self, features, captions):
        ...
        # (2)
        word_inputs = word_embedding_forward(captions_in, W_embed)

        # (3)
        if self.cell_type == 'rnn':
          hidden_states = rnn_forward(word_inputs, h_init, Wx, Wh, b)
    ...
    def sample(self, features, max_length=30):
        ...
        for t in range(max_length):
            # (1) Embed words → (N, W)
            word_vecs = W_embed[prev_words]

            # (2) RNN step → (N, H)
            next_h = rnn_step_forward(word_vecs, prev_h, Wx, Wh, b)
            prev_h = next_h

            # (3) Compute vocab scores → (N, V)
