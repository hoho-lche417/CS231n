...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        N = X.shape[0]
        X_2d = X.reshape((N, -1))
        X2, cache1 = affine_relu_forward(X_2d, self.params['W1'], self.params['b1'])
        ...
        dX_2d, dW1, db1 = affine_relu_backward(dX2, cache1)
        grads['W1'] = dW1 + self.reg * self.params['W1']
        grads['W2'] = dW2 + self.reg * self.params['W2']
        grads['b1'] = db1
        grads['b2'] = db2

        reg_term = (np.sum(self.params['W1'] ** 2) + np.sum(self.params['W2'] ** 2))
    ...
    def loss(self, X, y=None):
        ...
        for l in range(1, self.num_layers - 1):
            if l == 1:
                ...
                prev_X = outs[-1]
            out, cache = affine_relu_forward(prev_X, \
                self.params['W' + str(l)], self.params['b' + str(l)])
            outs.append(out)
            caches.append(cache)

        # last hidden layer
        scores, cache = affine_forward(outs[-1], \
        ...
        for l in range(self.num_layers - 2, 0, -1):
            dX, dW, db = affine_relu_backward(dX, caches[l - 1])
            fc_cache, _ = caches[l - 1]
            grads['W' + str(l)] = \
